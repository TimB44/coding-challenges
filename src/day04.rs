...
fn main() {
    ...
    let is_open = |row_i: usize, col_i: usize| -> bool {
        ...
        (start_row_i..end_row_i)
            ...
            .count()
            < SURROUNDED_THRESHOLD
    };
    let surrounded_count = grid
        .iter()
        .enumerate()
        .flat_map(|(row_i, row)| row.iter().enumerate().map(move |(col_i, _)| (row_i, col_i)))
        .filter(|&(row_i, col_i)| grid[row_i][col_i])
        .filter(|&(row_i, col_i)| is_open(row_i, col_i))
        .count();

    println!("Surrounded count = {surrounded_count}");
}
